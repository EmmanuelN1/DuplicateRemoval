...
def keep_most_detailed_whitespace_duplicate(csv_path):
    ...
    for idx in highlight_indices:
        excel_row = final_df.index.get_loc(idx) + 2  # +2 for header row
        for cell in ws[excel_row]:
            cell.fill = fill
    ...
    print(f"Unique duplicate groups: {best_duplicates.shape[0]}")
    print(f"Final rows: {len(final_df)}")
    print(f"Rows removed: {len(df) - len(final_df)}")

    print(f"\nTotal UNIQUE duplicates resolved: {len(best_duplicates)}")
    print(f"\nExcel file saved to:\n{output_path}")
